...
async def generation_agent(context_slice: dict, personalization_slice: dict, user_id: str = DEFAULT_USER_ID, exclude_keys: list = []) -> tuple[dict, float]:
    ...
    sys_msg = sys_prompt["content"] if sys_prompt else (
        ...
        "No canned AAC categories, no fixed menu sets, no resets to root. "
        "CRITICAL: Avoid making everything about food. Prioritize diverse needs, feelings, places, and activities. "
    ...
    hu_base = hu_prompt["content"] if hu_prompt else (
        ...
        "- Prioritize 'Life/Daily Needs', 'Feelings', and 'Activities' over food categories unless explicitly relevant.\n"
...
async def icon_agent(generated: dict) -> tuple[dict, float]:
    ...
    if items:
        ...
        sys_msg = sys_prompt["content"] if sys_prompt else (
            ...
            "   - For PLACES: Use landmark or building emojis (🏠, 🏙️, 🏥, 🌳).\n"
            "   - For ACTIONS: Use person-activity emojis (🏃, 🚶, 🧘, 🛀).\n"
