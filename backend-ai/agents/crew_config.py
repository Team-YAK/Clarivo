...
async def icon_agent(generated: dict) -> tuple[dict, float]:
    ...
    if items:
        ...
        sys_msg = sys_prompt["content"] if sys_prompt else (
            ...
            "   - For DESSERT/SWEETS/TREATS: Use 🍰🎂🧁🍪🍩🍫🍦🍮. NEVER use 🍠🥔🌽.\n"
            "   - For BEVERAGES: Use 🍵☕🥤🧃🧋🍷🍺🥛. NEVER use food emojis.\n"
            "   - For MEAT/PROTEIN: Use 🍖🥩🍗🥓🍤🦐. NEVER use vegetables.\n"
            "   - For VEGETABLES: Use 🥦🥬🥒🌽🥕🧅🥒. NOT sweets.\n"
            "   - For BREAKFAST: 🥐🥞🍳🥣. NEVER dinner items.\n"
            "   - For LUNCH/DINNER: 🍝🍕🍔🌮🍲. Match the specific cuisine.\n"
            "   - For EMOTIONS: Use specific faces (😢, 😄, 😠) combined with context (💔, ✨, 💢).\n"
            "   - For ATHLETICS/EXERCISE:\n"
            "     * RUN: 🏃 (Runner)\n"
            "     * WALK: 🚶 (Walker)\n"
            "     * LIFT/STRENGTH: 🏋️ (Weightlifter)\n"
            "     * YOGA/STRETCH: 🧘 (Person in Lotus Position) or 🙆 (Person Gesturing OK).\n"
            "     * DANCE: 💃 or 🕺 (Dancer).\n"
            "     * SWIM: 🏊 (Swimmer).\n"
            "     * BIKE/CYCLING: 🚲 or 🚴 (Cyclist).\n"
            "     * CLIMB: 🧗 (Climber).\n"
            "     * NEVER use 🌑 (Moon) for 'Stretch' or 'Exercise'.\n"
            "7. NEGATIVE CONSTRAINTS (NEVER DO THESE):\n"
            "   - NEVER use 🌑 for anything related to movement, new items, or stretching.\n"
            "   - NEVER use 🍠 for anything labeled 'sweet' or 'dessert'. It is a yam/potato.\n"
            "   - NEVER use 🙅 for 'good'. Use 👍 or ✅.\n"
            "   - NEVER use 🤑 or 💰 for 'mouth' or 'speaking'.\n"
            "   - NEVER use 🤣 for 'floor' or 'ground'.\n"
            "8. Use the Core Emoji Reference below as your source of truth — always prefer reference emojis when available.\n"
            "9. If uncertain about semantic fit, err toward MORE specific combinations (e.g., for 'coffee' use ☕ or ☕🫘, not just 🥤)."
        )
